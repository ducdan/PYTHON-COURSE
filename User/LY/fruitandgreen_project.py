...
# from flask_wtf import Form, TextField, Identity, AnonymousIdentity, identity_changed
from flask_sqlalchemy import SQLAlchemy, Model
from sqlalchemy import Column, DECIMAL, Integer, String, Text, CHAR, DATE, Float, ForeignKey
# from DataModel import XNK
...
def add_KHO( MAKHO, TENKHO, TENSP, DVT, SLTON, DIACHI, SDT):
    kho = KHO( MAKHO, TENKHO, TENSP, DVT, SLTON, DIACHI, SDT)
    db.session.add(kho)
    db.session.commit()

def add_PN(MAPN, SANPHAM, SLNHAP, SLNTHUC, DVT, CONTAINER_NO, NGAYNHAP, PRICE, TONGTIEN):
    pn = PHIEUNHAP(MAPN, SANPHAM, SLNHAP, SLNTHUC, DVT, CONTAINER_NO, NGAYNHAP, PRICE, TONGTIEN)
    db.session.add(pn)
    db.session.commit()

def add_PX(MAPX, NGAYDATHANG, NGAYGIAO, TENLOAISP,
                 TENSP, TRANGTHAI, SLXUAT, SLXTHUC,  DVT,
                 PRICE, TONGTIENPX):
    px = PHIEUXUAT(MAPX, NGAYDATHANG, NGAYGIAO, TENLOAISP,
                 TENSP, TRANGTHAI, SLXUAT, SLXTHUC,  DVT,
                 PRICE, TONGTIENPX)
    db.session.add(px)
    db.session.commit()
